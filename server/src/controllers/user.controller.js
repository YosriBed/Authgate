  ...
  switch (role) {
    case "teacher":
      ...
      home = {
        ...
        studentsCount: teacherCourses
          .map((course) => course.students.length)
          .reduce((a, b) => a + b, 0),
        resourcesCount: teacherCourses
          .map((course) => course.resources.length)
          .reduce((a, b) => a + b, 0),
        questionsCount: teacherCourses
          .map((course) => course.questions.length)
          .reduce((a, b) => a + b, 0),
      };
      break;
    case "student":
      studentCourses = await courseService.getAllCourses({
    ...
    case "admin":
      home = {
        ...
        resourcesCount:
          fs.readdirSync(path.resolve(process.cwd(), "src/uploads")).length - 1,
      };
      break;
    default:
